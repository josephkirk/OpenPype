...
class RootsEntity(DictImmutableKeysEntity):
    ...
    def _item_initalization(self):
        ...
        self.multiplatform = origin_schema_data.get("multiplatform", False)
        ...
        super(RootsEntity, self)._item_initalization()
    ...
    def schema_validations(self):
        ...
        if not isinstance(self.object_type, dict):
            ...
            raise EntitySchemaError(self, reason)
        super(RootsEntity, self).schema_validations()
    ...
    def set_override_state(self, state, ignore_missing_defaults):
        ...
        super(RootsEntity, self).set_override_state(
            state, True
        )
    ...
    def on_child_change(self, child_obj):
        ...
        return super(RootsEntity, self).on_child_change(child_obj)

    def _set_children_values(self, state):
        if state >= OverrideState.DEFAULTS:
            if self._default_value is NOT_SET:
                raise DefaultsNotDefined(self)

            for key, child_obj in self.non_gui_children.items():
                child_value = self._default_value.get(key, NOT_SET)
                child_obj.update_default_value(child_value)
